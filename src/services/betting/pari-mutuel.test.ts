...
describe('calculatePayouts', () => {
  ...
  it('should calculate payouts correctly with standard scenario', () => {
    ...
    expect(user1Result?.payout).toBeCloseTo(14.85, 2)
    expect(user1Result?.netGain).toBeCloseTo(4.85, 2)
    expect(user2Result?.payout).toBeCloseTo(29.70, 2)
    expect(user2Result?.netGain).toBeCloseTo(9.70, 2)
  ...
  it('should handle single bettor scenario', () => {
    const bets: BetWithUser[] = [
      { userId: 'user1', optionId: 'yes', amount: 50 }
    ]

    const result = calculatePayouts(bets, 'yes')

    expect(result).toEqual([
      { userId: 'user1', betAmount: 50, payout: 49.5, netGain: -0.5 }
    ])
  })
