...
export async function POST(
  ...
  try {
    ...
    if (outcome.event.status !== 'UPCOMING') {
      return NextResponse.json(
        { error: 'Betting is not allowed after event starts' },
        { status: 400 }
      )
    }
    ...
    if (outcome.status !== 'OPEN') {
      return NextResponse.json(
        { error: 'Outcome is not accepting bets' },
        { status: 400 }
      )
    }
    ...
    const isValidOption = outcome.options.some(
      option => option.id === validatedData.optionId
    )

    if (!isValidOption) {
      return NextResponse.json(
        { error: 'Invalid option for this outcome' },
        { status: 400 }
      )
    }
