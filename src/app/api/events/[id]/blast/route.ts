...
export async function POST(request: NextRequest, { params }: { params: Promise<{ id: string }> }) {
    try {
        ...
        if (members.length === 0) {
            return NextResponse.json({ error: 'No members found with the specified filter' }, { status: 404 });
        }
        ...
        return NextResponse.json({ error: 'Failed to send text blast' }, { status: 500 });
    }
}
