...
export async function DELETE(request: NextRequest, { params }: { params: Promise<{ id: string }> }) {
  try {
    ...
    if (bet.userId !== session.userId) {
      return NextResponse.json({ error: 'Can only delete your own bets' }, { status: 403 });
    }

    await checkAndUpdateStatus(bet.option.outcome.eventId);

    if (bet.option.outcome.event.status !== 'UPCOMING') {
      return NextResponse.json({ error: 'Cannot delete bets after event starts' }, { status: 400 });
    }
